...
class MultiTouchGestureDetector(
    ...
    /**
     * Returns number of fingers pressed.
     */
    var fingereCount = 0
     ...
     */
    fun onTouchEvent(event: MotionEvent, img: View?): Boolean {
        ...
        outer@ for (i in 0 until count) {
            ...
            inner@ for (j in i + 1 until count) {
                ...
                ) + MAX_ROTATION % MAX_ROTATION.toFloat()).toFloat()
                break@outer
            }
        }
        ...
        if (!isInProgress && (wasInProgress
                ...
                2.0
                    ...
                    Math.pow(
                        mCurrentFocusY - mInitialFocusY.toDouble(),
                        2.0
                    ) > mTouchSlopSquare)
        ) {
            mCurrentSpan = span
        ...
        Log.d(
            "inccoo",
            event.toString() + " " + action + " " + MotionEvent.ACTION_MOVE
        )
        ...
        if (action == MotionEvent.ACTION_MOVE) {
            ...
            if (isInProgress) {
                if (getScale() != NO_SCALE) {
                    Log.d("TAG", "getScale: " + count)
                    mListener.onScale(this, img)
                }
                if (getRotation() != NO_ROTATE) {
                    Log.d("TAG", "NO_ROTATE: " + count)
                    mListener.onRotate(this, img)
                }
                if (getMoveX() != NO_MOVE || getMoveY() != NO_MOVE) {
                    Log.d("TAG", "NO_MOVE: " + count)
                    mListener.onMove(this, img)
                }
            }
            mPreviousSpan = mCurrentSpan
            mPreviousFocusX = mCurrentFocusX
    ...
    interface OnMultiTouchGestureListener {
         ...
         * retrieve extended info about event state.
         */
        fun onEnd(detector: MultiTouchGestureDetector?, view: View?)
    }

    open class SimpleOnMultiTouchGestureListener :
